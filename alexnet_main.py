...
tf.app.flags.DEFINE_integer('max_steps',220*2, 'the max training steps ')
...
tf.app.flags.DEFINE_integer('decay_step', 22, 'Number of decay-step')
...
tf.app.flags.DEFINE_string('val_data_dir', 'H:/why_workspace/ReCTS/img_dir2/', 'the test dataset dir')
...
tf.app.flags.DEFINE_string('mode', 'validation2', 'Running mode. One of {"train", "validation", "test"}')
...
def train():
    ...
    with tf.Session() as sess:
        ...
        test_images, test_labels = test_feeder.input_pipeline(batch_size=FLAGS.batch_size, aug=False, num_epochs = 1)
...
def validation_output_txt():
    ...
    with tf.Session() as sess:
        ...
        graph = build_graph(1, is_train=False, is_test=True)
        sess.run(tf.global_variables_initializer())
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                # for _ in range(0, FLAGS.batch_size):
                #     print(linecache.getline(FLAGS.label_dir, final_predict_index[_][0] + 1))
