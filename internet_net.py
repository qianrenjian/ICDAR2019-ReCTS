...
tf.app.flags.DEFINE_integer('eval_steps', 5000, "the step num to eval")
tf.app.flags.DEFINE_integer('save_steps', 2000, "the steps to save")

tf.app.flags.DEFINE_string('checkpoint_dir', './checkpoint_internet/', 'the checkpoint dir')
tf.app.flags.DEFINE_string('train_data_dir', 'H:/why_workspace/ReCTS/img_dir2/', 'the train dataset dir')
tf.app.flags.DEFINE_string('test_data_dir', 'H:/why_workspace/ReCTS/img_dir2/', 'the test dataset dir')
tf.app.flags.DEFINE_string('log_dir', './log_internet', 'the logging dir')
...
class DataIterator:
    def __init__(self, data_dir):
        ...
        random.shuffle(self.image_names)    #将序列的所有元素随机排序。
    ...
    def data_augmentation(images):
        if FLAGS.random_flip_up_down:  #上下随机翻转
            images = tf.image.random_flip_up_down(images)
        if FLAGS.random_brightness:  #亮度变化
            images = tf.image.random_brightness(images, max_delta=0.3)
        if FLAGS.random_contrast:  #对比度变化
    ...
    def input_pipeline(self, batch_size, num_epochs=None, aug=False):
        ...
        images_content = tf.read_file(input_queue[0])  #文件中读取一行内容
        #tf.image.decode_png()解析这一行内容并将其转为张量列表
        #tf.image.convert_image_dtype()进行格式转化
        images = tf.image.convert_image_dtype(tf.image.decode_jpeg(images_content, channels=1), tf.float32)
        if aug:
            images = self.data_augmentation(images)
        new_size = tf.constant([FLAGS.image_size, FLAGS.image_size], dtype=tf.int32)
        images = tf.image.resize_images(images, new_size) #图像缩放
...
def train():
    ...
    with tf.Session() as sess:
        train_images, train_labels = train_feeder.input_pipeline(batch_size=FLAGS.batch_size, aug=True, num_epochs=1)
